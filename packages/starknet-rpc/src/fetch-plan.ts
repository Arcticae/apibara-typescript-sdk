...
export class FetchPlan {
  ...
  /** Normalized event addresses that can constrain an RPC event request. */
  readonly eventAddresses: readonly string[];
  ...
  constructor(filters: readonly Filter[]) {
    ...
    for (const filter of filters) {
      ...
      for (const event of filter.events ?? []) {
        ...
        if (event.address !== undefined) {
          eventAddresses.add(normalizeFelt(event.address));
        }
      }
    ...
    this.eventAddresses = Object.freeze([...eventAddresses]);
