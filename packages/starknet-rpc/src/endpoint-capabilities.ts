...
export class StarknetEndpointCapabilities {
  private constructor(
    /** Whether block zero can currently be queried from this endpoint. */
    readonly blockZeroAvailable: boolean,
  ...
  static async probe(
    ...
    const [blockZeroAvailable, batch, webSocket] = await Promise.all([
      probeBlockZeroAvailability(client),
      probeBatch(url, options),
      wsUrl
        ? probeWebSocketTransport(wsUrl, options.timeout)
        : Promise.resolve(false),
    ]);
    return new StarknetEndpointCapabilities(
      blockZeroAvailable,
      batch,
      webSocket,
    );
...
async function probeBlockZeroAvailability(
  client: StarknetJsonRpcClient,
): Promise<boolean> {
  try {
